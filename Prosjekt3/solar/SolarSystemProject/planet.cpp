...
double planet::distance(planet& otherPlanet){ //Also making another planet
    return norm(position-otherPlanet.position);
}
...
vec planet::GForce(planet& otherPlanet, double G_constant){
    ...
    double l = norm(cross(position - otherPlanet.position, velocity));
    ...
    if (relcheck)
    {
        ...
        return -((position - otherPlanet.position)/r)*((G_constant*(otherPlanet.mass)*this->mass)/(r*r))*(additionalterm);
    }
    else
    {
